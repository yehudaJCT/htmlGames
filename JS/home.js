...
function init() {
    ...
    if (localStorage.length == 0) {
        let maxScore = { catchTheTarget:0 ,ticTacToe:0};
...
//Change between divs in same place between log in and sign up. 
function switchVisible() {
    if (document.getElementById('containerLogIn')) {
        if (document.getElementById('containerLogIn').style.display == 'none') {
            ...
            document.getElementById('containerSignUp').style.display = 'flex';
        }
    }
}
//Change between divs in same place between log in and forgot. 
function switchVisible1() {
    if (document.getElementById('containerLogIn')) {
        if (document.getElementById('containerLogIn').style.display == 'none') {
            ...
            document.getElementById('containerLogIn').style.display = 'none';
            document.getElementById('containerForgot').style.display = 'flex';
        }
    }
}
...
function handleClick(event) {
    ...
    if (idStr == "logIn") {
        let suspendedUntil = localStorage.getItem('suspendedUntil');
        if (suspendedUntil && Date.now() < parseInt(suspendedUntil)) {
            document.getElementById("message").innerHTML = "Your account is suspended. Please try again later.";
            setTimeout(function () {
    ...
    if (idStr == "signIn") {
        ...
        if (!isExistUserName(userName)) {
            ...
            user.achivment = { date: 0, maxTicTacToe: 0, maxCatchTheTarget: 0 };
            ...
            users.push(user);
            alert(`Hi ${userName}. Welcome to Game World!!!`);
            open("../HTML/games.html");
            window.top.close();
        }
        else {
...
function suspendAccount() {
    let suspendedUntil = new Date(Date.now() + 10 * 60 * 1000); 
    localStorage.setItem('suspendedUntil', suspendedUntil.getTime());
}
